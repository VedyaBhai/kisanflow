...
import {
  ...
  Package, Sparkles, Route as RouteIcon, BadgeCheck, Info,
  Sprout, Factory, Scale, Languages, Mic
} from "lucide-react";
import {
...
import {
  CROPS, LOCATIONS, FARMERS, BUYERS, VEHICLES,
  DEMAND_HISTORY, DEMAND_MONTHS, DEMAND_RADAR, VIRTUAL_LOTS, BUYER_DEMANDS,
  DEMO_BUYER_DEMAND, DEMO_CANDIDATE_FARMERS,
  WHATSAPP_MESSAGE, TELUGU_VOICE_TEXT, DEMO_FARMER_RESPONSES, DEMO_LOT_ID, DEMO_LOGISTICS
} from "./data/mockData.js";
import { isSupabaseConfigured } from "./lib/supabase.js";
import { getFarmers } from "./services/farmers.js";
import { getBuyers } from "./services/buyers.js";
import { getVirtualLots } from "./services/virtualLots.js";
import { getBuyerDemands } from "./services/demands.js";
import { buildForecast, summarizeForecast } from "./services/forecast.js";
import { findEligibleFarmers } from "./services/eligibility.js";
import { openWhatsAppChatForFarmer, canWhatsAppFarmer } from "./services/whatsapp.js";
import { playTeluguVoice } from "./services/voice.js";
import { parseResponseQuantity } from "./services/farmerResponse.js";

/* ============================================================
   KISANFLOW — Demand-first agricultural supply chain prototype
...
function OverviewPage({ role, goTo, farmers, buyers, virtualLots }) {
    ...
    <div className="page">
      ...
      <section>
        ...
        <div className="grid-4">
          ...
          <Metric label="Open virtual lots" value={virtualLots.length} />
...
/* ================================================================
   Demand-to-delivery workflow (forecast -> farmers -> lot)
   Shared demo state lives in the app shell; these components
   render it using the existing card/table/pill styles.
   ================================================================ */

// Derives the demo virtual lot purely from which farmers have accepted —
// allocated and remaining are never tracked separately, so every surface
// showing this lot reads identical numbers.
function buildDemoLot(acceptedNames, eligible) {
  const contributors = eligible.filter((f) => acceptedNames.includes(f.name));
  const allocated = contributors.reduce((sum, f) => sum + f.qty, 0);
  const required = DEMO_BUYER_DEMAND.quantityKg;
  const remaining = Math.max(0, required - allocated);

  return {
    id: DEMO_LOT_ID,
    buyer: DEMO_BUYER_DEMAND.buyer,
    crop: DEMO_BUYER_DEMAND.crop,
    grade: DEMO_BUYER_DEMAND.grade,
    required,
    allocated,
    remaining,
    pct: Math.min(100, Math.round((allocated / required) * 100)),
    complete: remaining === 0 && allocated > 0,
    contributors,
  };
}

function FarmerNotificationsModal({ eligible, accepted, onAccept, onClose }) {
  const [status, setStatus] = useState({}); // per-farmer transient status line

  const setFarmerStatus = (name, text) =>
    setStatus((s) => ({ ...s, [name]: text }));

  const handleWhatsApp = (farmer) => {
    const result = openWhatsAppChatForFarmer(farmer.name, WHATSAPP_MESSAGE);
    if (result.ok) {
      // Wording is deliberate: the chat was opened, not delivered.
      setFarmerStatus(farmer.name, "WhatsApp opened · Press Send in WhatsApp");
    } else if (result.reason === "blocked") {
      setFarmerStatus(farmer.name, "Allow popups to open WhatsApp");
    } else {
      setFarmerStatus(farmer.name, "Add WhatsApp number to enable");
    }
  };

  const handleVoice = (farmer) => {
    const result = playTeluguVoice(TELUGU_VOICE_TEXT);
    if (!result.ok) {
      setFarmerStatus(farmer.name, "Voice playback not supported in this browser");
    } else if (!result.usedTeluguVoice) {
      setFarmerStatus(farmer.name, "Playing · no Telugu voice installed, using default");
    } else {
      setFarmerStatus(farmer.name, "Playing Telugu voice preview");
    }
  };

  const totalAvailable = eligible.reduce((sum, f) => sum + f.qty, 0);
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal modal-wide" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="opp-figs" style={{ marginBottom: 4 }}>
          <div><span className="fig-label">Eligible farmers</span><span className="fig-val">{eligible.length}</span></div>
          <div><span className="fig-label">Total available</span><span className="fig-val">{totalAvailable.toLocaleString("en-IN")} kg</span></div>
        </div>

        <div className="modal-body">
          {eligible.map((f) => {
            const isAccepted = accepted.includes(f.name);
            const numberOk = canWhatsAppFarmer(f.name);
            return (
              <div className="notif-card" key={f.id}>
                <div className="notif-head">
                  <div>
                    <b>{f.name}</b>
                    <span className="muted-sub"> · {f.loc}</span>
                  </div>
                  <div className="notif-figs">
                    <span>{f.qty.toLocaleString("en-IN")} kg</span>
                    <span className="notif-score">{f.score}%</span>
                  </div>
                </div>

                <div className="notif-reply">“{DEMO_FARMER_RESPONSES[f.name]}”</div>
                ...
                <div className="notif-actions">
                  <button
                    ...
                    disabled={!numberOk}
                    title={numberOk ? "Open WhatsApp chat" : "No WhatsApp number configured"}
                  >
                    {numberOk ? "WhatsApp" : "Add WhatsApp number"}
                  </button>
                  <button className="btn btn-ghost btn-sm" onClick={() => handleVoice(f)}>
                    Play Telugu Voice
                  </button>
                  <button
                ...
                {status[f.name] && <div className="notif-status">{status[f.name]}</div>}
              </div>
            );
          })}
        </div>
        ...
        <Disclaimer>
          KISANFLOW opens WhatsApp with the message pre-filled — the sender still presses Send.
          The Telugu voice preview plays locally in this browser and is not sent through WhatsApp.
        </Disclaimer>
      </div>
    </div>
  );
...
function DemoLotCard({ lot }) {
  return (
    <Card title="Virtual Lot" icon={Boxes} tone={lot.complete ? "green" : undefined}>
      <div className="lot-head">
        ...
        <StatusPill status={
          lot.complete
            ? { key: "VIRTUAL LOT COMPLETED", color: COLORS.balanced, bg: COLORS.balancedSoft }
            : { key: "COLLECTING", color: COLORS.amber, bg: COLORS.amberSoft }
        } />
      </div>

      {lot.contributors.length > 0 && (
        <table className="table">
          <thead><tr><th>Farmer</th><th>Location</th><th>Contribution</th></tr></thead>
          <tbody>
            {lot.contributors.map((f) => (
              <tr key={f.id}><td>{f.name}</td><td>{f.loc}</td><td>{f.qty.toLocaleString("en-IN")} kg</td></tr>
            ))}
          </tbody>
        </table>
      )}
      ...
      <div className="progress-row">
        <div className="progress-track"><div className="progress-fill" style={{ width: lot.pct + "%" }} /></div>
        <span className="progress-label">
          {lot.allocated.toLocaleString("en-IN")} / {lot.required.toLocaleString("en-IN")} kg
        </span>
      </div>

      <div className="opp-figs" style={{ marginTop: 14 }}>
        <div><span className="fig-label">Required</span><span className="fig-val">{lot.required.toLocaleString("en-IN")} kg</span></div>
        <div><span className="fig-label">Allocated</span><span className="fig-val">{lot.allocated.toLocaleString("en-IN")} kg</span></div>
        <div><span className="fig-label">Remaining</span><span className="fig-val">{lot.remaining.toLocaleString("en-IN")} kg</span></div>
        <div><span className="fig-label">Completion</span><span className="fig-val">{lot.pct}%</span></div>
      </div>
    </Card>
  );
}

function LogisticsHandoffCard() {
  return (
    <Card title="Logistics next step" icon={Truck} tone="green">
      <p className="opp-line">Lot complete — ready to hand off to pooled logistics.</p>
      <div className="opp-figs">
        <div><span className="fig-label">Logistics partner</span><span className="fig-val">{DEMO_LOGISTICS.partner}</span></div>
        <div><span className="fig-label">Vehicle</span><span className="fig-val">{DEMO_LOGISTICS.vehicle}</span></div>
        <div><span className="fig-label">Load</span><span className="fig-val">{DEMO_LOGISTICS.loadKg.toLocaleString("en-IN")} kg</span></div>
        <div><span className="fig-label">Destination</span><span className="fig-val">{DEMO_LOGISTICS.destination}</span></div>
      </div>
      <button className="btn btn-primary">Assign Pooled Logistics</button>
    </Card>
  );
}
...
function DemandRadarPage({ workflow }) {
  ...
  // Deterministic 7-day tomato forecast for the Hyderabad corridor.
  const forecast = useMemo(() => buildForecast(DEMAND_HISTORY.Tomato), []);
  const tomatoRow = DEMAND_RADAR.find((r) => r.crop === "Tomato" && r.loc === "Hyderabad");
  const summary = useMemo(
    () => summarizeForecast(forecast, tomatoRow ? tomatoRow.supply : 0),
    [forecast, tomatoRow]
  );

  // Historical months then forecast days on one continuous axis.
  const forecastChartData = useMemo(() => [
    ...DEMAND_MONTHS.map((m, i) => ({ label: m, Historical: DEMAND_HISTORY.Tomato[i] })),
    ...forecast.map((d) => ({ label: d.label, Forecast: d.demandKg })),
  ], [forecast]);
  ...
  return (
    <div className="page">
      ...
      {summary && (
        <Card
          title="AI Demand Forecast — Tomato · Hyderabad · Next 7 days"
          icon={Sparkles}
          tone="amber"
          action={<AILabel>Prototype Forecast</AILabel>}
        >
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={forecastChartData}>
              <CartesianGrid stroke={COLORS.line} vertical={false} />
              <XAxis dataKey="label" tick={{ fontSize: 11, fill: COLORS.ink }} axisLine={{ stroke: COLORS.line }} tickLine={false} />
              <YAxis tick={{ fontSize: 12, fill: COLORS.ink }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ borderRadius: 8, border: `1px solid ${COLORS.line}`, fontFamily: "IBM Plex Sans" }} />
              <Legend />
              <Line type="monotone" dataKey="Historical" stroke={COLORS.forest} strokeWidth={2.5} dot={false} connectNulls />
              <Line type="monotone" dataKey="Forecast" stroke={COLORS.amber} strokeWidth={2.5} strokeDasharray="5 4" dot={{ r: 3 }} connectNulls />
            </LineChart>
          </ResponsiveContainer>

          <div className="opp-figs" style={{ marginTop: 14 }}>
            <div><span className="fig-label">Forecast demand</span><span className="fig-val">{summary.forecastT} T</span></div>
            <div><span className="fig-label">Available supply</span><span className="fig-val">{summary.supplyT} T</span></div>
            <div><span className="fig-label">Expected gap</span><span className="fig-val" style={{ color: summary.shortage ? COLORS.brick : COLORS.balanced }}>{summary.gapT} T</span></div>
            <div>
              <span className="fig-label">Risk</span>
              <span className="fig-val">
                <StatusPill status={
                  summary.shortage
                    ? { key: summary.riskLabel, color: COLORS.brick, bg: COLORS.brickSoft }
                    : { key: summary.riskLabel, color: COLORS.balanced, bg: COLORS.balancedSoft }
                } />
              </span>
            </div>
            <div><span className="fig-label">Confidence</span><span className="fig-val">{summary.confidence}%</span></div>
          </div>

          <div className="before-after-mini" style={{ marginBottom: 12 }}>
            <div>
              <span className="fig-label">Buyer requirement</span>
              <span className="fig-val">
                {DEMO_BUYER_DEMAND.buyer} · {DEMO_BUYER_DEMAND.quantityKg.toLocaleString("en-IN")} kg Grade-{DEMO_BUYER_DEMAND.grade} {DEMO_BUYER_DEMAND.crop} · {DEMO_BUYER_DEMAND.destination} · {DEMO_BUYER_DEMAND.requiredDate}
              </span>
            </div>
          </div>

          {!workflow.matched ? (
            <button className="btn btn-primary" onClick={workflow.findFarmers}>
              <Users size={16} /> Find Eligible Farmers
            </button>
          ) : (
            <>
              <div className="opp-figs">
                <div><span className="fig-label">Eligible farmers</span><span className="fig-val">{workflow.eligible.length} of {DEMO_CANDIDATE_FARMERS.length} screened</span></div>
                <div><span className="fig-label">Available from matches</span><span className="fig-val">{workflow.totalAvailableKg.toLocaleString("en-IN")} kg</span></div>
              </div>
              <table className="table">
                <thead><tr><th>Farmer</th><th>Location</th><th>Quantity</th><th>Match score</th></tr></thead>
                <tbody>
                  {workflow.eligible.map((f) => (
                    <tr key={f.id}>
                      <td>{f.name}</td>
                      <td>{f.loc}</td>
                      <td>{f.qty.toLocaleString("en-IN")} kg</td>
                      <td>{f.score}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <p className="muted-sub" style={{ marginTop: 8 }}>
                Matched on crop match, quality match, quantity, distance, harvest readiness and price compatibility.
              </p>
              <button className="btn btn-primary" style={{ marginTop: 14 }} onClick={workflow.openNotifications}>
                Notify Eligible Farmers
              </button>
            </>
          )}
          <Disclaimer>
            Prototype forecast — derived deterministically from historical demand using a moving average,
            a fixed seasonal factor and a damped trend. Not a trained ML model.
          </Disclaimer>
        </Card>
      )}

      {workflow.matched && workflow.lot.allocated > 0 && <DemoLotCard lot={workflow.lot} />}
      {workflow.lot.complete && <LogisticsHandoffCard />}
      ...
      {workflow.notificationsOpen && (
        <FarmerNotificationsModal
          eligible={workflow.eligible}
          accepted={workflow.accepted}
          onAccept={workflow.acceptFarmer}
          onClose={workflow.closeNotifications}
        />
      )}
    </div>
  );
...
function FarmerPage({ farmers }) {
  const [showForm, setShowForm] = useState(false);
  const my = farmers[0] || FARMERS[0]; // demo farmer: Ramesh Reddy
...
function BuyerPage({ buyerDemands }) {
    ...
    <div className="page">
      ...
      <Card title="Active requests" icon={Boxes}>
        <table className="table">
          ...
          <tbody>
            {buyerDemands.map((d) => {
              const isMatched = d.status === "MATCHED" || d.status === "READY FOR LOGISTICS";
              const st = isMatched
                ? { key: "MATCHED", color: COLORS.balanced, bg: COLORS.balancedSoft }
                : { key: "COLLECTING", color: COLORS.amber, bg: COLORS.amberSoft };
              return (
                <tr key={d.id}>
                  <td>{d.crop}</td>
                  <td>{d.qty.toLocaleString("en-IN")} kg</td>
                  <td><StatusPill status={st} /></td>
                  <td>{d.matchedPct != null ? `${d.matchedPct}%` : "—"}</td>
                  <td>{d.delivery || "—"}</td>
                </tr>
              );
            })}
...
function VirtualLotsPage({ virtualLots, demoLot }) {
  return (
    <div className="page">
      <div className="page-head">
        <h1>Virtual Lots</h1>
      ...
      {/* Live demo lot — same derived values the Demand Radar workflow shows. */}
      {demoLot && demoLot.allocated > 0 && <DemoLotCard lot={demoLot} />}
...
function FarmersDirectoryPage({ farmers }) {
  return (
    <div className="page">
...
function BuyersDirectoryPage({ buyers }) {
  return (
    <div className="page">
...
export default function KisanflowApp() {
  ...
  // Data layer: starts on bundled demo data so the app renders instantly
  // with zero configuration. If Supabase is configured, real data quietly
  // replaces it once fetched — every page reads from this same state, so
  // there's a single source of truth instead of separate hardcoded copies.
  const [farmers, setFarmers] = useState(FARMERS);
  const [buyers, setBuyers] = useState(BUYERS);
  const [virtualLots, setVirtualLots] = useState(VIRTUAL_LOTS);
  const [buyerDemands, setBuyerDemands] = useState(BUYER_DEMANDS);
  const [dataSource, setDataSource] = useState("demo");

  /* Demand-to-delivery demo workflow state.
     Held here (not inside a page) so the demo lot stays consistent
     across Demand Radar and the Virtual Lots page, and survives
     navigating between them mid-demo. */
  const [matched, setMatched] = useState(false);
  const [notificationsOpen, setNotificationsOpen] = useState(false);
  const [accepted, setAccepted] = useState([]);

  const eligibility = useMemo(
    () => findEligibleFarmers(DEMO_BUYER_DEMAND, DEMO_CANDIDATE_FARMERS),
    []
  );

  const demoLot = useMemo(
    () => buildDemoLot(accepted, eligibility.farmers),
    [accepted, eligibility.farmers]
  );

  const acceptFarmer = useCallback((farmer) => {
    // Quantity comes from parsing the farmer's own reply, not from a
    // hardcoded number — the parser is local and deterministic.
    const parsed = parseResponseQuantity(DEMO_FARMER_RESPONSES[farmer.name]);
    if (parsed == null) return;
    setAccepted((prev) => (prev.includes(farmer.name) ? prev : [...prev, farmer.name]));
  }, []);

  const workflow = useMemo(() => ({
    matched,
    eligible: eligibility.farmers,
    totalAvailableKg: eligibility.totalAvailableKg,
    accepted,
    lot: demoLot,
    notificationsOpen,
    findFarmers: () => setMatched(true),
    openNotifications: () => setNotificationsOpen(true),
    closeNotifications: () => setNotificationsOpen(false),
    acceptFarmer,
  }), [matched, eligibility, accepted, demoLot, notificationsOpen, acceptFarmer]);

  useEffect(() => {
    if (!isSupabaseConfigured) return; // stay on demo data, no network call
    let cancelled = false;

    (async () => {
      const [f, b, vl, bd] = await Promise.all([
        getFarmers(), getBuyers(), getVirtualLots(), getBuyerDemands(),
      ]);
      if (cancelled) return;
      setFarmers(f);
      setBuyers(b);
      setVirtualLots(vl);
      setBuyerDemands(bd);
      // Each service returns the exact same mock array reference when it
      // falls back, so this comparison detects whether anything came from
      // a real, populated Supabase table.
      const anyLive = f !== FARMERS || b !== BUYERS || vl !== VIRTUAL_LOTS || bd !== BUYER_DEMANDS;
  ...
  const PageComponent = useMemo(() => {
    switch (page) {
      case "overview": return <OverviewPage role={role} goTo={goTo} farmers={farmers} buyers={buyers} virtualLots={virtualLots} />;
      case "radar": return <DemandRadarPage workflow={workflow} />;
      case "farmers": return role === "Farmer" ? <FarmerPage farmers={farmers} /> : <FarmersDirectoryPage farmers={farmers} />;
      case "buyers": return role === "Buyer" ? <BuyerPage buyerDemands={buyerDemands} /> : <BuyersDirectoryPage buyers={buyers} />;
      case "lots": return <VirtualLotsPage virtualLots={virtualLots} demoLot={demoLot} />;
  ...
  }, [page, role, goTo, farmers, buyers, virtualLots, buyerDemands, workflow, demoLot]);
  ...
  return (
    <div className="kf-root">
      <style>{`
        ...
        .role-btn:disabled { opacity:0.55; cursor:default; }
        ...
        .data-badge { font-size:10px; font-weight:700; padding:3px 9px; border-radius:20px; letter-spacing:0.3px; border:1px solid; }
        .data-badge.live { color:${COLORS.balanced}; background:${COLORS.balancedSoft}; border-color:${COLORS.balanced}33; }
        .data-badge.demo { color:${COLORS.amber}; background:${COLORS.amberSoft}; border-color:${COLORS.amber}33; }
        ...
        .modal-wide { width:560px; max-height:88vh; overflow-y:auto; }
        .notif-card { border:1px solid ${COLORS.line}; border-radius:10px; padding:12px 14px; background:${COLORS.bg}; }
        .notif-head { display:flex; align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap; }
        .notif-figs { display:flex; align-items:center; gap:10px; font-size:13px; color:${COLORS.forestDeep}; }
        .notif-score { font-weight:700; color:${COLORS.balanced}; }
        .notif-reply { font-size:12.5px; color:#5C6350; margin:8px 0 10px; }
        .notif-actions { display:flex; gap:8px; flex-wrap:wrap; }
        .notif-actions .btn:disabled { opacity:0.55; cursor:default; }
        .notif-status { font-size:11.5px; color:${COLORS.amber}; margin-top:8px; }
      ...
      <div className="main">
        <div className="topbar">
          ...
          <div className="muted-sub" style={{ fontWeight: 600, color: COLORS.forestDeep, display: "flex", alignItems: "center", gap: 8 }}>
            {NAV.find((n) => n.key === page)?.label}
            <span className={`data-badge ${dataSource === "live" ? "live" : "demo"}`}>
              {dataSource === "live" ? "LIVE DATA" : "DEMO DATA"}
            </span>
          </div>
          <div className="role-switch">
            {["Farmer", "Buyer", "Admin"].map((r) => {
              // Only the Admin POV may switch views — Farmer/Buyer are
              // read-only "public" perspectives on this login-less demo.
              // "Admin" itself always stays clickable so a switched view
              // can return. No visual change: same three buttons, same
              // styling, always shown.
              const canClick = role === "Admin" || r === "Admin";
              return (
                <button
                  key={r}
                  className={`role-btn ${role === r ? "active" : ""}`}
                  onClick={() => canClick && setRole(r)}
                  disabled={!canClick}
                  title={canClick ? undefined : "Switch back to Admin to change view"}
                >
                  {r}
                </button>
              );
